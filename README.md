...
- **🎯 Bet on · 长期布局**：找出**可能持续一年以上**的上涨行情，回答三件事 —— ① **现在能不能买**
  （买点档 `T1 启动前埋伏` / `T2 山腰确认` / `T3 加速已走远` / `T4 观察` + 0~100 买入评分）；
  ② **大概还能走多久**（按该股**自身历史**的上涨段分布推算，单位自动切 周 / 月 / 年）；
  ③ **什么时候卖**（该股自己的目标价 P25/P50/P75 区间 + 趋势减半线 / 清仓线 / 过热减仓线 / 回撤纪律线 / 时间纪律线）。
  9 项因子加权 + 5 类乘法折扣（过热 / 假启动 / 波动过热 / 短历史 / 震荡区），折扣只进排序不进判档；
  另有 23 个行业 → 上游期货的映射，带产业因果方向（写明**相关≠因果**）。
  **诚实边界（必读）**：本模块素材为两份长周期量化研究档案与两轮独立复核，**最终结论都是 RESEARCH_REJECTED** ——
  run1 的 Deflated Sharpe 仅 0.8107（< 0.95）、Haircut 后夏普从 1.25 腰斩到 0.64；run2 的「提前埋伏」净增量
  只有 +1.49pp、8 年里 4 年为负；横截面复核更发现**位置类因子对未来 120 日收益是负向的**（与「半山腰入场」
  直觉相反）。因此本模块只把研究中**唯一在任何口径下都没被否掉**的「状态基线」当主锚，自我定位是
  **筛选漏斗 + 纪律工具，不假装能预测**。主榜之外另设**观察榜**（宽松轨，界面带显著警示条「⚠ 宽松轨 · 不是推荐」，
  **绝不与主榜混排**）。长历史走 `data/hist.db`（1990 起、前复权），全市场约 23 秒
...
- **特殊概念（名字玄学）**：A 股每年都会轮动的「无厘头题材」——日期彩头（9·18「就要发」× 中华「华」字辈）、
  ...
  判定分**三档**：**🔴 触发**（组内整体跑赢全市场 或 批量冲板且跟涨家数扩散）／
  **🟡 异动**（涨停密度高但**跟涨家数未扩散** —— 少数个股冲板，**不做方向结论、不进信号**）／
  **⚪ 观察**（未达门槛）。两个关键设计：① 密度档用**宽度**（组内上涨家数占比 − 全市场上涨家数占比）守门，
  而不是拿均涨去比一个接近 0 的小数 —— 否则 −0.16% 与 +0.01% 会得出相反结论（**刀锋效应**，无统计意义）；
  ② 窗口类概念（生肖 / 日期彩头）**无热度时不占表格**，合并成一行提示，避免「全年在列、0 涨停」的白噪音。
  明细只列**真正在动的票**（大涨 或 量比 ≥ 1.5），僵尸票不占版面。规则表是纯数据结构，可自行增删；
  已下线的规则连同**下线原因**一起记在 `SPECIAL_RETIRED` / `SPECIAL_DATES_RETIRED` 里，防止后人误当漏写补回。
  **仅为情绪观察提示，不是买入建议**。
